...
<body class="font-sans antialiased bg-[#fdf6ee] overflow-x-hidden">
    <div class="flex min-h-screen">
        ...
        <aside class="fixed left-0 top-0 z-40 h-screen w-64 bg-[#fcf3e6] border-r border-[#f2e6d8] py-6 px-4 overflow-y-auto">
            <div class="flex flex-col h-full">
                ...
                <nav class="flex flex-col flex-1 gap-1">
                    ...
                    {{-- <a href="#" class="flex gap-3 items-center px-4 py-2 text-base font-medium text-gray-700 rounded-lg hover:bg-blue-50">
                    ...
                    @if (auth()->user()->isSuperAdmin() || auth()->user()->isAdmin())
                        <a href="{{ route('admin.history.index') }}" class="flex items-center gap-3 px-4 py-2 rounded-lg font-medium text-base {{ request()->routeIs('admin.history.*') ? 'bg-blue-900 text-white' : 'text-gray-700 hover:bg-blue-50' }}">
                ...
                <!-- Profil utilisateur -->
                <div class="flex gap-3 items-center px-2 mt-8">
                    @php
                        $user = Auth::user();
        ...
        <div class="flex flex-col flex-1 ml-64 min-h-screen">
            <!-- Header fixe -->
            <header class="flex fixed top-0 right-0 left-64 z-30 justify-between items-center px-10 py-4 bg-white shadow-sm">
                ...
                <div class="flex gap-6 items-center">
                    {{-- <button class="relative">
                        ...
                        <span class="absolute -top-1 -right-1 px-1.5 py-0.5 text-xs font-bold text-white bg-blue-600 rounded-full">5</span>
                    </button> --}}
                    <!-- Dropdown menu profil -->
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" @keydown.escape="open = false" type="button" class="flex gap-2 items-center focus:outline-none" :aria-expanded="open" aria-haspopup="true">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 left-auto z-50 mt-2 w-56 max-w-xs bg-white rounded border border-gray-200 shadow-lg" style="display: none; overflow-x: hidden;">
                            <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50">Profil</a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="px-4 py-2 w-full text-left text-gray-700 hover:bg-blue-50">Déconnexion</button>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <main class="overflow-y-auto flex-1 pt-16 bg-gray-50">
                <!-- Notifications -->
                @if(session('success'))
                    <div class="relative px-4 py-3 mx-4 mt-4 mb-4 text-green-700 bg-green-100 rounded border border-green-400" role="alert">
                ...
                @if(session('error'))
                    <div class="relative px-4 py-3 mx-4 mt-4 mb-4 text-red-700 bg-red-100 rounded border border-red-400" role="alert">
                ...
                @if(session('warning'))
                    <div class="relative px-4 py-3 mx-4 mt-4 mb-4 text-yellow-700 bg-yellow-100 rounded border border-yellow-400" role="alert">
                        <span class="block sm:inline">{{ session('warning') }}</span>
                        <span class="absolute top-0 right-0 bottom-0 px-4 py-3">
                            <svg class="w-6 h-6 text-yellow-500 fill-current" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                <title>Close</title>
                                <path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z"/>
                            </svg>
                        </span>
                ...
                @if(session('info'))
                    <div class="relative px-4 py-3 mx-4 mt-4 mb-4 text-blue-700 bg-blue-100 rounded border border-blue-400" role="alert">
